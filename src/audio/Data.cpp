...
// one in Stream. Unfortunately, abstracting this to allow for reading from streams and
...
static T readNumber(const unsigned char *source, int &i, int length) {
    ...
    for (int s = 0; s <= end; s += 8) {
        if (i >= length) {
            delete[] source;
            throw std::runtime_error("Audio file corrupted");
        }
        result |= source[i++] << s;
    }
    return result;
...
// Throw an std::runtime_error with `err` as the description if the sections don't match.
void strEq(const unsigned char *source, int &i, int length, const char *cstr, const char *err) {
...
static unsigned char *loadSoundFile(const std::string &path, int &length) {
    ...
    if (!file.good()) {
        delete[] output;
        length = 0;
        throw std::runtime_error("Failed to read from audio file");
    }
...
dan::audio::Data doLoadVorbis(stb_vorbis *handle) {
    if (handle == NULL) {
        throw std::runtime_error("Failed to load ogg data");
    }
...
dan::audio::Data dan::audio::loadWavFile(const std::string &path) {
    ...
    Data sd(loadWavMemory(data, length));
    delete[] data;
    return sd;
}
dan::audio::Data dan::audio::loadWavMemory(const unsigned char *data, int length) {
    ...
    if (readInt(data, i, length) != 16) { // Check format
        throw std::runtime_error("WAV Corrupted");
    }
    ...
    for (int f = 0; f < 4; i++) {
        if (i >= length) {
            throw std::runtime_error("Wav file corrupted, failed to seek to \"data\" chunk");
        }
        if ("data"[f] == data[i]) {
            f++;
        }
    ...
    return Data(
        std::shared_ptr<Data::data_t>(soundData, freeData<char>),
        dataLengthBytes,
        frequency,
