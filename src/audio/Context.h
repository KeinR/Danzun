#ifndef DANZUN_AUDIO_CONTEXT_H_INCLUDED
#define DANZUN_AUDIO_CONTEXT_H_INCLUDED
...
namespace dan::audio {
    class Context {
        ...
        void close();
        void steal(Context &other);
        // Won't copy over bindings, etc. of course
        void copy(const Context &other);
    public:
        Context(const Device &parent);
        Context(Context &&other);
        Context(const Context &other); // see copy()
        ~Context();
        Context &operator=(Context &&other);
        Context &operator=(const Context &other); // see copy()
